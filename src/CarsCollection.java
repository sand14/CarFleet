...
public class CarsCollection
{
	...
	public void updatemanufacturername(Car c,String man){
		Manufacturer manufacturer2;
		String oldmanufacturer=c.getManufacturer();
		int index= -1;
		for (int i = 0; i < manufacturer.length; i++)
		{
			if (manufacturer[i].getManufacturerName().equalsIgnoreCase(man))
				index = i;
		}
		if (index == -1)
		{
			if (manufacturer.length < maxManufacturers)
			{
				manufacturer2 = new Manufacturer(man, c);
				addManufacturer(manufacturer2);
				for (int i = 0; i < manufacturer.length; i++)
				{
					if (manufacturer[i].getManufacturerName().equalsIgnoreCase(oldmanufacturer))
						manufacturer[i].deleteCar(c);
				}
			}
		}
		else
	...
	public void checkmanufacturerempty(){
		for (int i = 0; i < manufacturer.length; i++)
		{
			if(manufacturer[i].carCount()==0)
				removeManufacturer(manufacturer[i]);
		}
	}
	public void removeCar(Car c){
		...
		manufacturer[index].deleteCar(c);
	}
	...
	private void removeManufacturer(Manufacturer man)
	{
		int index = 0;
		String name= man.getManufacturerName();
		for(int i=0;i< manufacturer.length;i++)
		{
			if(manufacturer[i].getManufacturerName().equalsIgnoreCase(name))
				index=i;
		}
		for(int i=index;i< manufacturer.length-1;i++)
			manufacturer[i]=manufacturer[i+1];
		manufacturer= Arrays.copyOf(manufacturer,manufacturer.length-1);
	}
	...
	public Car[] getAllCars()
	{
		...
		return CarFleetSystem.vectorToCar(result);
	}
	...
	public Car[] search(String manufacturer){
		...
		for(int i=0; i<car.length ; i++)
		{
			manufacturers = car[i].getManufacturer();

			if(manufacturer.equals(manufacturers))
			{
				result.addElement(car[i]);
			}
		}
