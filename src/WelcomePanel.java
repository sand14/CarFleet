...
public class WelcomePanel extends JPanel implements ChangeListener
{
	...
	private JLabel headingLabel = new JLabel("Welcome to the Car Fleet Management System", JLabel.CENTER);
	...
	private JLabel totalKmLabel = new JLabel();
	...
	public WelcomePanel(CarFleetSystem carSys, String data)
	{
		carSystem = carSys;
		file = data;
	...
	private void updateStats()
	{
		...
		int cars = (int)carSystem.getStatistics(CarFleetSystem.CARS_COUNT);
		int manufacturers = (int)carSystem.getStatistics(CarFleetSystem.MANUFACTURERS_COUNT);
		double avgKm = Math.floor(carSystem.getStatistics(CarFleetSystem.AVERAGE_DISTANCE) * 10 + 0.5) / 10;
		double avgAge = Math.floor(carSystem.getStatistics(CarFleetSystem.AVERAGE_AGE) * 10 + 0.5) / 10;
		double totalKm = Math.floor(carSystem.getStatistics(CarFleetSystem.TOTAL_KM) * 10 + 0.5) / 10;
		...
		totalKmLabel.setText("Total car kilometers: " + String.valueOf(totalKm));
